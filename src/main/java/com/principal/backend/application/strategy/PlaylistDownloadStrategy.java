...
@Component
public class PlaylistDownloadStrategy implements DownloadStrategy {

    private final PythonDownloadClient pythonClient;
    private final GoogleDriveClient driveClient;

    public PlaylistDownloadStrategy(PythonDownloadClient pythonClient, GoogleDriveClient driveClient) {
    ...
    public DownloadResult execute(String url, String accessToken) {
        try {
            ...
            return new DownloadResult("success", "Playlist downloaded and uploaded successfully", file.getName());
        } catch (Exception e) {
